...
class ChunkManager:
    ...
    def analyze_document(self, pages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if needs_chunking:
            # Calculate pages per chunk based on actual token density
    ...
    def create_chunks(self, pages: List[Dict[str, Any]]) -> Tuple[List[List[Dict[str, Any]]], Dict[str, Any]]:
        """
        Create document chunks for processing using dynamic token-based chunking.
        ...
        # Progress tracking
        logger.info(f"Starting dynamic chunk creation for {len(pages)} pages")
        ...
        try:
            ...
            max_tokens = analysis['usable_limit']
            overlap_ratio = analysis.get('chunk_overlap_ratio', 0.1)

            # Use dynamic chunking based on actual page token counts
            logger.info(f"Creating dynamic chunks with max {max_tokens:,} tokens per chunk")
            chunks = self.token_estimator.create_dynamic_chunks(
                pages, max_tokens, overlap_ratio
            )
            ...
            for i, chunk in enumerate(chunks):
                chunk_tokens = self.token_estimator.estimate_tokens_from_pages(chunk)
                chunk_info.append({
                    'chunk_index': i + 1,
                    'pages': len(chunk),
                    'tokens': chunk_tokens
                })
                logger.debug(f"Chunk {i+1}: {len(chunk)} pages, ~{chunk_tokens:,} tokens")
            
            # Update analysis
            analysis['actual_chunks'] = len(chunks)
            analysis['chunk_details'] = chunk_info
    ...
    def merge_chunk_results(self, chunk_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        valid_results = [r for r in chunk_results if r is not None and isinstance(r, dict)]
        
        if not valid_results:
            logger.error("All chunk results are invalid")
            return self._create_empty_result()
        
        # Log failed chunks
        failed_chunks = len(chunk_results) - len(valid_results)
        if failed_chunks > 0:
            logger.warning(f"Skipping {failed_chunks} failed chunk(s) during merge")

        if len(valid_results) == 1:
            return valid_results[0]

        logger.info(f"Merging results from {len(valid_results)} valid chunks (out of {len(chunk_results)} total)")
        ...
        # Process each chunk's results (using valid_results instead of chunk_results)
        for chunk_idx, chunk_result in enumerate(valid_results):
        ...
        self._resolve_cross_chunk_entries(all_patient_groups, valid_results)

        # Create consolidated result
        consolidated_result = self._create_consolidated_result(
            valid_results, all_patient_groups
        )
        
        # Add error information if some chunks failed
        if failed_chunks > 0:
            consolidated_result['chunking_warnings'] = {
                'failed_chunks': failed_chunks,
                'successful_chunks': len(valid_results),
                'total_chunks': len(chunk_results)
            }

        logger.info(f"Consolidated into {len(all_patient_groups)} unique patients from {len(valid_results)} chunks")
