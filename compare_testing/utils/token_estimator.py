...
class TokenEstimator:
    ...
    def calculate_pages_per_chunk(self,
        ...
        """
        Calculate how many pages should be in each chunk (legacy method for backward compatibility).
    ...
    def create_dynamic_chunks(self,
                            ...
                            content_type: str = 'ocr_text') -> List[List[Dict[str, Any]]]:
        """
        Create chunks dynamically based on actual page token counts.
        Always estimates tokens from actual text content for accuracy.
        ...
        # Pre-calculate token count for each page for accurate chunking
        page_tokens_map = {}
        for i, page in enumerate(pages):
            page_text = page.get('text', '')
            page_tokens_map[i] = self.estimate_tokens_from_text(page_text, content_type)
        ...
        for i, page in enumerate(pages):
            ...
            if current_chunk and current_tokens + page_tokens > safe_limit:
                ...
                # Calculate overlap pages
                overlap_tokens = int(max_tokens_per_chunk * overlap_ratio)
                overlap_chunk = []
                overlap_total = 0
                
                # Add pages from end of previous chunk for overlap
                for prev_idx in range(len(current_chunk) - 1, -1, -1):
                    prev_page = current_chunk[prev_idx]
                    prev_page_idx = pages.index(prev_page)
                    prev_tokens = page_tokens_map[prev_page_idx]
                    
                    if overlap_total + prev_tokens <= overlap_tokens:
                        overlap_chunk.insert(0, prev_page)
                        overlap_total += prev_tokens
                    else:
                        break
                
                # Start new chunk with overlap pages
        ...
        # Add the last chunk if it has pages
        if current_chunk:
            chunks.append(current_chunk)
    ...
    def get_optimal_chunk_config(self, model_name: str) -> Dict[str, Any]:
        ...
        configs = {
            'qwen_2_5_7b_instruct_patient': {
                'max_tokens_per_chunk': int(model_limit * 0.60),  # 60% of 32K limit (~19,200 tokens) - more conservative
                'overlap_ratio': 0.1,  # 10% overlap
                'safety_buffer': 0.40   # 40% safety buffer (very conservative for stability)
            },
            'gpt-oss-20b_patient': {
