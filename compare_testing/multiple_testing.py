        ...
        try:
            ...
            if result:
                ...
                if processed_in_chunks:
                    ...
                    chunking_warnings = result.get('chunking_warnings', {})
                    if chunking_warnings:
                        failed = chunking_warnings.get('failed_chunks', 0)
                        successful = chunking_warnings.get('successful_chunks', 0)
                        if failed > 0:
                            print(f"⚠️  Warning: {failed} chunk(s) failed, {successful} successful")
