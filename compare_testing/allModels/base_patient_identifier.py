...
class BasePatientIdentifier(abc.ABC):
    ...
    def call_model_with_chunking(self, pages: List[Dict[str, Any]],
        ...
        # Update display with actual chunk count
        actual_chunk_count = len(chunks)
        print(f"📦 Created {actual_chunk_count} chunks (adjusted from estimate)")

        for i, chunk in enumerate(chunks):
            self.logger.info(f"Processing chunk {i+1}/{actual_chunk_count} ({len(chunk)} pages)")
            print(f"🔄 Processing chunk {i+1}/{actual_chunk_count} ({len(chunk)} pages)...")
            ...
            try:
                ...
                if chunk_result and isinstance(chunk_result, dict):
                    ...
                    # CRITICAL FIX: Don't mark as successful if it's an empty result
                    has_patients = chunk_result.get('summary', {}).get('total_patients', 0) > 0
                    has_structure = 'patient_groups' in chunk_result or 'summary' in chunk_result
                    is_error_result = chunk_result.get('summary', {}).get('total_patients', 0) == 0 and \
                                     chunk_result.get('summary', {}).get('total_entries', 0) == 0
                    
                    if has_structure and not is_error_result:
                        chunk_results.append(chunk_result)
                        patients_count = chunk_result.get('summary', {}).get('total_patients', 0)
                        print(f"✅ Chunk {i+1} completed successfully ({patients_count} patients)")
                    else:
                        self.logger.warning(f"Chunk {i+1} returned empty or error result")
                        failed_chunks.append(i+1)
                        print(f"⚠️ Chunk {i+1} returned empty result")
                else:
    ...
    def process_single_chunk(self, pages: List[Dict[str, Any]],
        ...
        # Get model limit for validation
        model_limits = token_estimator.get_model_context_limits()
        model_limit = model_limits.get(self.algorithm_name.lower(), 8192)
        ...
        try:
            ...
            processing_time = time.time() - start_time
            # Return None to signal failure instead of empty result
            return None
        except Exception as e:
